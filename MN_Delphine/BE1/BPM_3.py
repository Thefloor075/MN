...
def a(i,j):
	return inv_dx * ( A(i,j) * inv_dx - B(i,j) * 0.5 * inv_dt - C(i,j)*0.5)

def b(i,j):
	return inv_dx*(- 2*A(i,j) * inv_dx)  + D(i,j)

def c(i,j):
	return (A(i,j) * inv_dx - B(i,j) *inv_dt - C(i,j)*0.5)* inv_dx

def d(i,j):
	if i == 0:
		return B(i,j) * 0.5 * inv_dx * inv_dt * (- E_final[i+1][j-1] + Ext)
	if i == n_X-1:
		return B(i,j) * 0.5 * inv_dx * inv_dt * (- Ext + E_final[i-1][j-1])
	return  B(i,j) * 0.5 * inv_dx * inv_dt * (- E_final[i+1][j-1] + E_final[i-1][j-1])
